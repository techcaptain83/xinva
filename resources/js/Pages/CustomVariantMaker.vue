...
export default {
    ...
    data() {
        return {
            ...
            vm_prediction_output:'',
            vm_prediction_loading: false
    ...
    methods: {
        ...
        axios.post(route('custom.variant-maker.store'), formData)
            .then(response => {
                // Handle the response as needed
                if(response.data.success) {
                    this.vm_prediction_id = response.data.vm_prediction_id
                    const intervalId = setInterval(() => {
                        axios.get(route("variant-maker.prediction", this.vm_prediction_id)).then((response) => {
                        if (response.data.output) {
                            clearInterval(intervalId);
                            this.vm_prediction_loading = false;
                            this.vm_prediction_output = response.data.output;
                        }
                    });
                    }, 10000);
                }
    ...
    <DashboardLayout>
        <div class="flex vh-100 row">
            ...
            <div class="col-md-9 col-sm-12 h-100 image-content">
                <div class="flex flex-col justify-center md:max-w-[100%] p-4 text-white xl:flex-row">
                    <div class="align-items-center col-md-9 flex h-14 justify-around md:mx-auto position-relative rounded-md">
                        ...
                        <div class="col-3 flex justify-content-center justify-end mr-4 offset-9 position-absolute">
                            <button v-if="!vm_prediction_loading" type="submit" @click.prevent="submit($event); vm_prediction_loading=true;"
                ...
                <div v-if="!vm_prediction_loading && vm_prediction_output != ''"
                    class="flex flex-grow-0 img-list">
                    <div v-for="(img, key) in vm_prediction_output" class="p-4 w-full">
                        <div class="mt-6">
                            <div class="flex justify-end rounded-lg hero" v-bind:style="{
                            ...
                            <div class="flex gap-4 justify-center my-3">
                                <a
                                    :href="
                                        '/prediction-results/' + key + '/variant-maker/scale'
                                    "
                ...
                <!-- The Modal -->
                <PreviewModal v-if="showModal" @close="showModal = false" :image-url="imageUrl" :show_buttons=false>
                </PreviewModal>
