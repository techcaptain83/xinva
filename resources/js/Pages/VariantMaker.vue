...
// background start //
const vm_prediction_id = ref("");
const vm_prediction_loading = ref(true);
const vm_prediction_check_interval = ref(null);
const vm_prediction_output = ref("");

const variantMaker = async () => {
    axios.get(route("variant-maker.prediction", props.vm_prediction_id)).then((response) => {
        console.log(response);
        if (response.data.completed_at !== null) {
            clearInterval(vm_prediction_check_interval.value);
            vm_prediction_loading.value = false;
            vm_prediction_output.value = response.data.output;
            vm_prediction_id.value = response.data.vm_prediction_id;
            // window.location.href = '/prediction-results/'+vm_prediction_id.value+'/remove-bg/scale';
        }
    });
};
...
onMounted(() => {
    ...
    vm_prediction_check_interval.value = setInterval(variantMaker, 10000);
});
</script>
<script>
export default {
...
<template>
    ...
    <DashboardLayout>
        ...
        >
            <div class="w-100">
                <div
                    v-if="!vm_prediction_loading && vm_prediction_output != ''"
                    class="flex flex-wrap justify-content-center mt-10 gap-10">
                    <div v-for="(img, key) in vm_prediction_output"
                        class="flex flex-wrap justify-center"
                    >
                        <div class="position-relative">

                            <svg @click="showModal = true; imageUrl = img;" fill="#ffff" version="1.1" id="Layer_1"
                            ...
                            <img v-bind:src="img" class="h-w-512 rounded-lg ">
                            <div class="flex gap-4 justify-center my-3">
                                <Link
                                :href="'/prediction-results/'+ key +'/variant-maker/scale'"
                ...
                <div v-if="vm_prediction_loading">
                    <div
                        role="status"
