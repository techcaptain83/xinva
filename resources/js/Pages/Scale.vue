...
// scale start //
const scale_prediction_id = ref("");
const scale_prediction_loading = ref(true);
const scale_prediction_check_interval = ref(null);
const scale_prediction_output = ref("");

const scalingImage = async () => {
    await axios
        .get(route("scale-image", props.scale_prediction_id))
        .then((response) => {
            console.log(response);
            if (response.data.success && response.data.output != null) {
                clearInterval(scale_prediction_check_interval.value);
                scale_prediction_loading.value = false;
                scale_prediction_output.value = response.data.output;
                scale_prediction_id.value = response.data.scale_prediction_id;
                window.location.href = '/scale-prediction-result/' +
                                        scale_prediction_id.value +
                                        '/download';
...
onMounted(() => {
    ...
    scale_prediction_check_interval.value = setInterval(scalingImage, 3000);
});
</script>
<script>
export default {
...
<template>
    ...
    <DashboardLayout>
        ...
        <div
            class="flex flex-col lg:flex-row max-w-[70%] mx-auto justify-center"
        >
            <div class="">
                <div
                    v-if="!scale_prediction_loading && prediction_output != ''"
                ...
                >
                    <div class="flex flex-wrap justify-center">
                        <div class="mt-6">
                            <div
                                class="flex justify-end rounded-lg hero h-w-512"
                                v-bind:style="{
                                    backgroundImage:
                                        'url(' + scale_prediction_output + ')',
                                }"
                            >
                            ...
                            <div class="flex gap-4 justify-center my-3">
                                <a
                                    :href="
                                        ...
                                        '/download'
                                    "
                                    class="block h-10 flex items-center gap-2 rounded-lg border border-blue-600 bg-indigo-600 px-3 py-3 text-sm font-medium text-white hover:bg-indigo-600 hover:text-white focus:outline-none focus:ring active:text-opacity-75 bg-linear"
                                    href="/"
                                >
                                    Download
                                    <svg
                                ...
                                <!-- <a
                                    ...
                                    href="#"
                                    :data-img="scale_prediction_output"
                                >
                                    Preview
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                ...
                <div v-if="scale_prediction_loading">
                    <div
                        role="status"
                        class=" text-white flex-col flex items-center mt-60 justify-center"
                    >
                    <span>Hmm, Please Wait While Xinva Increase The Resolution to 4X</span>
                    <!-- <button
                            disabled
                            type="button"
                            class="mt-2 bg-[#5145CD] hover:bg-[#5145CD] focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 inline-flex items-center"
                        >
                            <svg
                                aria-hidden="true"
                                role="status"
